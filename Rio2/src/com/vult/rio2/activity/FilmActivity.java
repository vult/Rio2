package com.vult.rio2.activity;
...
import com.vult.rio2.R;
import com.vult.rio2.customview.SlidingMenuCustom;
import com.vult.rio2.listener.MenuSlidingClickListener;
import com.vult.rio2.utils.Utils;
...
public class FilmActivity extends YouTubeBaseActivity implements
	...
	public static String VIDEO_ID = "bB0UIAtF9Nw";
	AdView adView;
	...
	public void onShareClickListener() {
		try {
			...
			sendIntent
					.putExtra(
							Intent.EXTRA_TEXT,
							"This is a good app to watch full Rio 2 free !."
									+ " Enjoy it now and get the interested."
									+ "https://play.google.com/store/apps/details?id=com.vult.rio2");
	...
	public void onClick(View v) {
		...
		if (v == mBtSession1) {
			intent.putExtra("YOUTUBE_ID", "bB0UIAtF9Nw");
			startActivity(intent);
			finish();
		} else if (v == mBtSession2) {
			intent.putExtra("YOUTUBE_ID", "4SXfVyrpwxI");
			startActivity(intent);
			finish();
		} else if (v == mBtSession3) {
			intent.putExtra("YOUTUBE_ID", "S6pmwv307-s");
			startActivity(intent);
			finish();
		} else if (v == mBtSession4) {
			intent.putExtra("YOUTUBE_ID", "iJ1eE8rRQME");
			startActivity(intent);
			finish();
		} else if (v == mIvClose) {
